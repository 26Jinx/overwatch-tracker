...
router.get('/analysis', (_req: Request, res: Response) => {
  ...
  const rows = db.prepare(`
    SELECT m.id, m.hero, m.sens, m.dpi, m.win, a.overall_acc, a.crit_acc, a.feel
...
router.get('/', (_req: Request, res: Response) => {
  ...
  const rows = (db.prepare(`
    SELECT m.id, m.date, m.time, m.hero, m.role, m.map, m.game_type, m.queue_mode, m.win, m.sens,
           m.dpi, m.blind_trial, m.blind_set_id, m.stage_index, m.revealed,
           a.overall_acc, a.crit_acc, a.hero_stat_label, a.hero_stat_value, a.feel, a.notes,
...
router.post('/', (req: Request, res: Response) => {
  const db = getDb();
  const { match_id, overall_acc, crit_acc, hero_stat_label, hero_stat_value, feel, notes,
  ...
  db.prepare(`
    INSERT INTO aim_stats (match_id, overall_acc, crit_acc, hero_stat_label, hero_stat_value, feel, notes,
                           elims, final_blows, deaths, damage, duration_min)
    VALUES (:match_id, :overall_acc, :crit_acc, :hero_stat_label, :hero_stat_value, :feel, :notes,
            :elims, :final_blows, :deaths, :damage, :duration_min)
